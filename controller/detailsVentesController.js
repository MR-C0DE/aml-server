...
class DetailsVentesController {
  ...
  static async createDetailsVente(request, response) {
    ...
    try {
      const result = await DetailsVentes.insertDetailsVente(id_vente, id_produit, quantite, prix_unitaire);
      response.status(201).json({ message: "Details vente created successfully.", id: result.insertId });
    } catch (error) {
      console.error(error);
  ...
  static async updateDetailsVente(request, response) {
    ...
    try {
      const result = await DetailsVentes.updateDetailsVente(id, id_vente, id_produit, quantite, prix_unitaire);
      if (result.affectedRows === 0) {
        response.status(404).json({ message: `Details vente with ID ${id} not found.` });
      } else {
        response.status(200).json({ message: "Details vente updated successfully." });
      }
    } catch (error) {
      console.error(error);
  ...
  static async deleteDetailsVente(request, response) {
    ...
    try {
      ...
      if (result.affectedRows === 0) {
        ...
        response.status(200).json({ message: "Details vente deleted successfully." });
      }
    } catch (error) {
      console.error(error);
  ...
  static async getDetailsVenteByVenteId(request, response) {
    ...
    try {
      const detailsVentes = await DetailsVentes.getDetailsVenteByVenteId(venteId);
      response.status(200).json(detailsVentes);
    } catch (error) {
      console.error(error);
