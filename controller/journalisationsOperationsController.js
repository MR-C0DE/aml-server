...
class JournalisationsOperationsController {
  static async getJournalisationsOperations(request, response) {
    try {
      const journalisations = await JournalisationsOperations.selectJournalisationsOperations();
  ...
  static async createJournalisationOperation(request, response) {
    try {
      const { date_operation, id_utilisateur, type_operation, description } = request.body;
      const result = await JournalisationsOperations.insertJournalisationOperation(date_operation, id_utilisateur, type_operation, description);
      response.status(201).json({ message: "Journalisation operation created successfully", id: result.insertId });
    } catch (error) {
      console.error(error);
  ...
  static async updateJournalisationOperation(request, response) {
    try {
      ...
      const { date_operation, id_utilisateur, type_operation, description } = request.body;
      await JournalisationsOperations.updateJournalisationOperation(id, date_operation, id_utilisateur, type_operation, description);
      response.status(200).json({ message: "Journalisation operation updated successfully" });
    } catch (error) {
      console.error(error);
  ...
  static async deleteJournalisationOperation(request, response) {
    try {
      ...
      response.status(200).json({ message: "Journalisation operation deleted successfully" });
    } catch (error) {
      console.error(error);
  ...
  static async getJournalisationOperationById(request, response) {
    try {
      const { id } = request.params;
      const journalisation = await JournalisationsOperations.getJournalisationOperationById(id);
      response.status(200).json(journalisation);
    } catch (error) {
      console.error(error);
      response.status(500).send("Error getting journalisation operation by ID.");
    }
  }
}
