...
class VentesController {
  ...
  static async createVente(request, response) {
    const { date_vente, montant_total, id_succursale, id_employe } = request.body;
    try {
      const result = await Ventes.insertVente(date_vente, montant_total, id_succursale, id_employe);
      response.status(201).json({ message: "Vente created successfully", venteId: result.insertId });
    } catch (error) {
      console.error(error);
  ...
  static async updateVente(request, response) {
    ...
    const { date_vente, montant_total, id_succursale, id_employe } = request.body;
    try {
      await Ventes.updateVente(venteId, date_vente, montant_total, id_succursale, id_employe);
      response.status(200).json({ message: `Vente with ID ${venteId} updated successfully` });
    } catch (error) {
      console.error(error);
  ...
  static async deleteVente(request, response) {
    ...
    try {
      ...
      response.status(200).json({ message: `Vente with ID ${venteId} deleted successfully` });
    } catch (error) {
      console.error(error);
  ...
  static async getVenteById(request, response) {
    ...
    try {
      ...
      if (vente) {
        ...
        response.status(404).json({ message: `Vente with ID ${venteId} not found` });
      }
    } catch (error) {
      console.error(error);
